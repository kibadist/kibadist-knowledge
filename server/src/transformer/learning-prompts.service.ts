...
export interface LearningPromptsInput {
  ...
  /**
   * Concept candidates the prompts may link to (may be empty). Normalized to
   * `{id, label, definition}` so BOTH the DET-283 per-section candidates and the
   * DET-351 whole-article candidates can be offered as linking targets — the
   * caller maps each candidate kind into this shape.
   */
